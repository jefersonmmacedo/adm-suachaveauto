...
export function EditAuto() {
    ...
    const {updateAuto, newFeature} = useContext(AuthContext);
    ...
    useEffect(() => {
        async function loadInfosAuto() {
            await api.get(`/Auto/${id}`).then((res) => {
    ...
    const idv4 = uuidv4();
    const idAuto = idv4.substring(0,8);
...
function handleNewAuto() {
    updateAuto({
        ...
        availability, bedroom, garage, suite, restroom, priceSale, priceRent, textRent, condominium, pets, furnished, newAuto: newProperties, firstLease,
    ...
    {characteristcs.map((item) => {
        return (
            ...
            <h4>Imagens</h4>
                       ...
                       <MyButtonComponent id={idAuto} uploadFiles2={uploadFiles2}/>
            ...
            <h4>Mais informações</h4>
                    ...
                    <button className="btnSend" onClick={handleNewAuto}>Atualizar anúncio</button>
                    </div>
            </div>
        </div>
    )
